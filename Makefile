...
PROTOC_VERSION := 21.6
PROTOC_DOWNLOAD_URL := https://github.com/protocolbuffers/protobuf/releases/download/v$(PROTOC_VERSION)/protoc-$(PROTOC_VERSION)-$(TARGET_OS)-$(TARGET_ARCH).zip
$(PROTOC): $(TOOLS_BIN_DIR)/protoc.zip
	curl -L $(PROTOC_DOWNLOAD_URL) -o $(TOOLS_BIN_DIR)/protoc.zip
	cd $(TOOLS_BIN_DIR); unzip -o protoc.zip
	mv -f $(TOOLS_BIN_DIR)/bin/protoc $(PROTOC)
...
cron/internal/data/request.pb.go: cron/internal/data/request.proto | $(PROTOC) $(PROTOC_GEN_GO)
	PATH=${PATH}:$(TOOLS_BIN_DIR) $(PROTOC) --plugin=$(PROTOC_GEN_GO) --go_out=../../../ cron/internal/data/request.proto
cron/internal/data/metadata.pb.go: cron/internal/data/metadata.proto | $(PROTOC) $(PROTOC_GEN_GO)
	PATH=${PATH}:$(TOOLS_BIN_DIR) $(PROTOC) --plugin=$(PROTOC_GEN_GO) --go_out=../../../ cron/internal/data/metadata.proto
